...
function setup_framework_options(){
    ...
    // Default: Options
    $args['menu_title'] = __('Flexform Options', Redux_TEXT_DOMAIN);

    // Set a custom page title for the options page.
    // Default: Options
    $args['page_title'] = __('Flexform Theme Options', Redux_TEXT_DOMAIN);
    ...
    $sections[] = array(
    				...
    				'fields' => array(
    					...
    					array(
    						...
    						'desc' => '',
    						'std' => "&copy;[the-year] Flexform &middot; Built by <a href='http://www.swiftpsd.com'>Swift Ideas</a> &middot; Powered by [wp-link]"
    						),
    					array(
    						'id' => 'show_backlink',
    						'type' => 'button_set',
    				...
    				'fields' => array(
    					...
    					array(
    						...
    						'options' => array(
    							...
    							'Sidebar-2'		=> 'Sidebar Two',
    							'Sidebar-3'		=> 'Sidebar Three',
    							'Sidebar-4'		=> 'Sidebar Four',
    							'Sidebar-5'		=> 'Sidebar Five',
    							'Sidebar-6'		=> 'Sidebar Six',
    							'Sidebar-7'		=> 'Sidebar Seven',
    							'Sidebar-8'		=> 'Sidebar Eight',
    						),
    						'desc' => '',
    						'std' => 'Sidebar-1'
    						),
    ...
    $sections[] = array(
    				...
    				'fields' => array(
    					...
    					array(
    						...
    						'sub_desc' => __('Paste the css here that can be found from step 2 of the FontDeck instructions (<a href="http://themes.swiftpsd.com/shared/flexform/fontdeck_step2.png" class="view" target="_blank">view</a>). NOTE: Make sure you provide the JS code in the box at the bottom for this/all FontDeck fonts you want to use.', Redux_TEXT_DOMAIN),
    						'desc' => '',
    						'std' => ''
    						),
    					array(
    					...
    					array(
    						...
    						'sub_desc' => __('Paste the js code here that can be found from step 1 of the FontDeck instructions (<a href="http://themes.swiftpsd.com/shared/flexform/fontdeck_step1.png" class="view" target="_blank">view</a>).', Redux_TEXT_DOMAIN),
    						'desc' => '',
    						'std' => ''
    						),
    					)
    ...
    $system_status = '<div class="redux-opts-section-desc">';
    $system_status .= '<p class="description">When logging a support ticket, please copy and paste the below into the comment form, to enable us to see information that may help us help you quicker.</p>';
    $system_status .= '<div class="system-status-list"><ul>';
    $system_status .= '<li><strong>Home URL:</strong>'.home_url().'</li>';
    $system_status .= '<li><strong>Site URL:</strong>'.site_url().'</li>';
    $system_status .= '<li><strong>Theme Name:</strong>'.$item_uri.'</li>';
    $system_status .= '<li><strong>Theme Version:</strong>'.$version.'</li>';
    if ( is_multisite() ) {
    $system_status .= '<li><strong>WordPress Version:</strong>'. 'WPMU ' . get_bloginfo('version').'</li>';
	} else {
    $system_status .= '<li><strong>WordPress Version:</strong>'. 'WP ' . get_bloginfo('version').'</li>';	
	}   
    $system_status .= '<li><strong>Web Server Info:</strong>'.esc_html( $_SERVER['SERVER_SOFTWARE'] ).'</li>';
    if ( function_exists( 'phpversion' ) ) {
    $system_status .= '<li><strong>PHP Version:</strong>'. esc_html( phpversion() ).'</li>';
    }
    if (function_exists( 'wp_convert_bytes_to_hr' )) {
	    $system_status .= '<li><strong>WP Memory Limit:</strong>';
	    $mem_limit = sf_let_to_num( WP_MEMORY_LIMIT );
	    
		if ( $mem_limit < 67108864 ) {
		$system_status .= '<mark class="error">' . wp_convert_bytes_to_hr( $mem_limit ) .' - We recommend setting memory to at least 64MB. Please see: <a href="http://codex.wordpress.org/Editing_wp-config.php#Increasing_memory_allocated_to_PHP">Increasing memory allocated to PHP</a></mark>';
		} else {
		$system_status .= '<mark class="yes">' . wp_convert_bytes_to_hr( $mem_limit ) . '</mark>';
		}
	    $system_status .= '</li>';
	    $system_status .= '<li><strong>WP Max Upload Size:</strong>'. wp_convert_bytes_to_hr( wp_max_upload_size() ) .'</li>';
	}
    if ( function_exists( 'ini_get' ) ) {
    $system_status .= '<li><strong>PHP Time Limit:</strong>'. ini_get('max_execution_time') .'</li>';
    }
    if ( defined('WP_DEBUG') && WP_DEBUG ) {
    $system_status .= '<li><strong>WP Debug Mode:</strong><mark class="yes">Yes</mark></li>';
    } else {
    $system_status .= '<li><strong>WP Debug Mode:</strong><mark class="no">No</mark></li>';    
    }
    $system_status .= '</ul></div>';
    $system_status .= '</div>';   
    ...
    $tabs['documentation'] = array(
    				...
    				'content' => '<p>Documentation for Flexform is available here <a href="http://themes.swiftpsd.com/documentation/flexform" target="_blank">here</a>.</p>'
    				);
    ...
    $tabs['support'] = array(
    				...
    				'content' => '<p>If you need support for Flexform, please visit our support site <a href="http://support.swiftpsd.com" target="_blank">here</a>.</p>'
    				);				
    ...
    $tabs['translation_plugin'] = array(
    				...
    				'content' => '<p>Flexform is 100% translation and multi-lingual ready, and we recommend that you use the very popular WPML plugin.</p>
    				...
    				<ol>
    					<li>Open the /flexform/languages/ folder and duplicate the en_US.po file, then rename it to your desired language code. For example; for German you need to re-name it as de_DE.po for Spanish es_ES.po for Turkish tr_TR.po etc. You can find all the codes from this link <a href="http://codex.wordpress.org/WordPress_in_Your_Language" target="_blank">http://codex.wordpress.org/WordPress_in_Your_Language</a></li>
    ...
    $tabs['system_status'] = array(
        'icon' => 'dashboard',
        'icon_class' => 'icon-large',
        'title' => __('System Status', Redux_TEXT_DOMAIN),
